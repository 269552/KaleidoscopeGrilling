...
public final class KnifeDropHandler {
    ...
    public static void onLivingDrops(LivingDropsEvent event){
        ...
        if(event.getEntity().getType()==EntityType.CHICKEN){
            ...
            drop(event,ModItems.SQUID_TENTACLE.get(),2+player.getRandom().nextInt(2)+randomBonus(player,looting),0.5F);
        }
    }
    private static int randomBonus(Player player,int looting){return looting<=0?0:player.getRandom().nextInt(looting+1);}
    private static void drop(LivingDropsEvent event,Item item,int count,float chance){if(event.getEntity().getRandom().nextFloat()<chance)event.getDrops().add(new ItemEntity(event.getEntity().level(),event.getEntity().getX(),event.getEntity().getY(),event.getEntity().getZ(),new ItemStack(item,count)));}
