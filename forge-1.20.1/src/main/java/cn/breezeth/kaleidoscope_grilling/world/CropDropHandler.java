...
public final class CropDropHandler {
  public static void onBlockBreak(BlockEvent.BreakEvent event) {
    ...
    if (id == null
        || (!id.toString().equals("kaleidoscope_cookery:straw_hat")
            && !id.toString().equals("kaleidoscope_cookery:straw_hat_flower"))) return;
    int fortune =
        EnchantmentHelper.getItemEnchantmentLevel(
            Enchantments.BLOCK_FORTUNE, event.getPlayer().getMainHandItem());
    tryDrop(
        event,
        new ItemStack(
    ...
    tryDrop(
        event,
        new ItemStack(
            ModItems.SWEET_POTATO.get(), 1 + event.getPlayer().getRandom().nextInt(1 + fortune)));
    tryDrop(
        event,
        new ItemStack(
  ...
  private static void tryDrop(BlockEvent.BreakEvent event, ItemStack stack) {
    if (event.getPlayer().getRandom().nextFloat() < 0.125F)
      Block.popResource((net.minecraft.world.level.Level) event.getLevel(), event.getPos(), stack);
  }
