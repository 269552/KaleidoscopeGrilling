...
public final class AdvancedRackScreen extends AbstractContainerScreen<AdvancedRackMenu> {
    ...
    private static final ResourceLocation TEXTURE = new ResourceLocation(
            KaleidoscopeGrilling.MOD_ID, "textures/gui/advanced_rack_settings.png");
    ...
    protected void renderBg(GuiGraphics graphics, float partialTick, int mouseX, int mouseY) {
        ...
        graphics.blit(depositHovered ? DEPOSIT_SELECTED_TEXTURE : DEPOSIT_TEXTURE,
                leftPos + 89, topPos + 65, 0, 0, 16, 16, 16, 16);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (button == 0 && isDepositAt(mouseX, mouseY) && minecraft != null && minecraft.gameMode != null) {
            minecraft.gameMode.handleInventoryButtonClick(menu.containerId, AdvancedRackMenu.DEPOSIT_BUTTON);
            return true;
        }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (compartment >= 0 && menu.slots.get(compartment).getItem().isEmpty() && !menu.getFilter(compartment).isEmpty()) {
    ...
    private boolean isDepositAt(double mouseX, double mouseY) {
        return mouseX >= leftPos + 89 && mouseX < leftPos + 105
                && mouseY >= topPos + 65 && mouseY < topPos + 81;
    }
