...
public final class SeasoningTooltip {
    ...
    public static void appendUnfinished(ItemStack stack, List<Component> tooltip) {
        ...
        tooltip.add(Component.translatable("tooltip.kaleidoscope_grilling.seasoning.capacity",
                ingredients.size(), CAPACITY).withStyle(ChatFormatting.GRAY));
        if (ingredients.isEmpty()) {
            tooltip.add(Component.translatable("tooltip.kaleidoscope_grilling.seasoning.place_and_shake")
                    .withStyle(ChatFormatting.DARK_GRAY));
            return;
        }
        List<String> missing = REQUIRED.stream().filter(id -> !ingredients.contains(id)).toList();
        if (missing.isEmpty()) {
            tooltip.add(Component.translatable("tooltip.kaleidoscope_grilling.seasoning.ready_to_shake")
                    .withStyle(ChatFormatting.YELLOW));
        } else {
            tooltip.add(Component.translatable("tooltip.kaleidoscope_grilling.seasoning.missing_required",
                    joinedNames(missing)).withStyle(ChatFormatting.RED));
        }
    ...
    public static void appendFinished(ItemStack stack, List<Component> tooltip) {
        ...
        tooltip.add(Component.translatable("tooltip.kaleidoscope_grilling.seasoning.uses_full",
                remaining, stack.getMaxDamage()).withStyle(ChatFormatting.GRAY));
        tooltip.add(Component.translatable("tooltip.kaleidoscope_grilling.seasoning.shift_details")
                .withStyle(ChatFormatting.GOLD));
        if (!Screen.hasShiftDown()) return;
        List<String> ingredients = SeasoningData.get(stack);
        tooltip.add(Component.translatable("tooltip.kaleidoscope_grilling.seasoning.provided_effects")
        ...
        if (effects.isEmpty()) {
            tooltip.add(Component.literal("- ").append(Component.translatable(
                    "hud.kaleidoscope_grilling.seasoning.no_effect")).withStyle(ChatFormatting.AQUA));
        } else {
            effects.forEach(effect -> tooltip.add(Component.literal("- ").append(effect)
                    .withStyle(ChatFormatting.AQUA)));
        }
        appendIngredients(ingredients, tooltip);
    }
    ...
    private static void appendIngredients(List<String> ingredients, List<Component> tooltip) {
        tooltip.add(Component.translatable("tooltip.kaleidoscope_grilling.seasoning.added_ingredients")
                .withStyle(ChatFormatting.DARK_GRAY));
        for (String id : ingredients) {
            tooltip.add(Component.literal("- ").append(item(id).getDescription())
                    .withStyle(ChatFormatting.DARK_GRAY));
        }
    }
