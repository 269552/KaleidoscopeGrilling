...
public final class RackShortcutScreen extends AbstractContainerScreen<RackShortcutMenu> {
    ...
    private static final ResourceLocation DEPOSIT_TEXTURE = new ResourceLocation(
            KaleidoscopeGrilling.MOD_ID, "textures/gui/advanced_rack_deposit.png");
    private static final ResourceLocation DEPOSIT_SELECTED_TEXTURE = new ResourceLocation(
    ...
    @Override public boolean mouseClicked(double x, double y, int button) {
        ...
        double logicalX = logicalMouseX(x, progress), logicalY = logicalMouseY(y, progress);
        if (button == 0 && isDepositAt(logicalX, logicalY)) { send(RackShortcutMenu.DEPOSIT_BUTTON); onClose(); return true; }
        int slot = at(logicalX, logicalY);
        if (button == 0 && slot >= 0 && menu.slots.get(slot).hasItem()) { select(slot); return true; }
    ...
    @Override public boolean keyPressed(int key, int scan, int modifiers) {
        if (minecraft != null) for (int i = 0; i < 9; i++) {
            if (minecraft.options.keyHotbarSlots[i].matches(key, scan) && menu.slots.get(i).hasItem()) { select(i); return true; }
        }
    ...
    @Override public boolean keyReleased(int key, int scan, int modifiers) {
        if (RackKeyHandler.KEY.matches(key, scan)) {
            ...
            if (depositHovered) { send(RackShortcutMenu.DEPOSIT_BUTTON); onClose(); }
            else if (hovered >= 0 && menu.slots.get(hovered).hasItem()) select(hovered); else onClose();
            return true;
        }
    ...
    private int at(double mouseX,double mouseY){for(int i=0;i<9;i++){int x=leftPos+menu.slots.get(i).x,y=topPos+menu.slots.get(i).y;if(mouseX>=x&&mouseX<x+16&&mouseY>=y&&mouseY<y+16)return i;}return-1;}
    ...
    private void drawCount(GuiGraphics graphics,int slot,int count){if(menu.getFilter(slot).isEmpty()&&menu.slots.get(slot).getItem().isEmpty())return;String text=Integer.toString(count);int x=leftPos+menu.slots.get(slot).x+16-font.width(text),y=topPos+menu.slots.get(slot).y+8;graphics.drawString(font,text,x,y,0xFFFFFFFF,true);}
