...
public final class RackShortcutScreen extends AbstractContainerScreen<RackShortcutMenu> {
  private static final ResourceLocation TEXTURE =
      new ResourceLocation(KaleidoscopeGrilling.MOD_ID, "textures/gui/advanced_rack_shortcut.png");
  ...
  public boolean mouseClicked(double x, double y, int button) {
    ...
    if (button == 0 && isDepositAt(logicalX, logicalY)) {
      send(RackShortcutMenu.DEPOSIT_BUTTON);
      onClose();
      return true;
    }
    int slot = at(logicalX, logicalY);
    if (button == 0 && slot >= 0 && menu.slots.get(slot).hasItem()) {
      select(slot);
      return true;
    }
  ...
  public boolean keyPressed(int key, int scan, int modifiers) {
    if (minecraft != null)
      for (int i = 0; i < 9; i++) {
        if (minecraft.options.keyHotbarSlots[i].matches(key, scan) && menu.slots.get(i).hasItem()) {
          select(i);
          return true;
        }
      }
  ...
  public boolean keyReleased(int key, int scan, int modifiers) {
    if (RackKeyHandler.KEY.matches(key, scan)) {
      ...
      if (depositHovered) {
        send(RackShortcutMenu.DEPOSIT_BUTTON);
        onClose();
      } else if (hovered >= 0 && menu.slots.get(hovered).hasItem()) select(hovered);
      else onClose();
      return true;
    }
  ...
  private int at(double mouseX, double mouseY) {
    for (int i = 0; i < 9; i++) {
      int x = leftPos + menu.slots.get(i).x, y = topPos + menu.slots.get(i).y;
  ...
  private void drawCount(GuiGraphics graphics, int slot, int count) {
  ...
  private void drawDeposit(GuiGraphics graphics) {
    int x = leftPos + 89, y = topPos + 64;
    graphics.blit(
        depositHovered ? DEPOSIT_SELECTED_TEXTURE : DEPOSIT_TEXTURE, x, y, 0, 0, 16, 16, 16, 16);
  }
    ...
    for (int i = 0; i < 9; i++) {
      ...
      if (!slot.getItem().isEmpty())
        SkewerOutlineRender.renderItem(
            ...
            leftPos + slot.x,
            topPos + slot.y,
            SkewerGuiDecorator.colorFor(slot.getItem()));
    }
  }
