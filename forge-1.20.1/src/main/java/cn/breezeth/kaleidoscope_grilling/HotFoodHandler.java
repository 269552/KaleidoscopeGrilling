...
public final class HotFoodHandler {
  private static final Map<LivingEntity, Map<MobEffect, Integer>> EFFECTS_BEFORE_USE =
      new WeakHashMap<>();
  ...
  public static void onTooltip(ItemTooltipEvent event) {
    ...
    if (event.getEntity() != null
      ...
      long until = event.getItemStack().getTag().getLong("HotUntil");
  ...
  public static void finishEarly(ItemStack stack, LivingEntity entity) {
    Map<MobEffect, Integer> before = EFFECTS_BEFORE_USE.remove(entity);
  ...
  private static void doubleNewDurations(LivingEntity entity, Map<MobEffect, Integer> before) {
    for (MobEffectInstance effect : List.copyOf(entity.getActiveEffects())) {
      if (effect.getEffect() == ModEffects.INVINCIBLE.get()) continue;
