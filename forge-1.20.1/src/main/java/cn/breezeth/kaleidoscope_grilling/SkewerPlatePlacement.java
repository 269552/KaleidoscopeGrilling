...
public final class SkewerPlatePlacement {
    public static void onRightClickBlock(PlayerInteractEvent.RightClickBlock event) {
        ...
        if (mainHand.is(ModItems.SKEWER_RECIPE_BOOK.get()) || mainHand.getItem() instanceof RecipeItem) return;
        ...
        if (SkewerPlateItem.isSkewer(held)) {
            InteractionResult result = place(event.getLevel(), event.getPos(), event.getFace(), event.getEntity(), held,
        ...
        if (!event.getLevel().isClientSide
                && SkeweringHandler.disassemble(offhand, event.getEntity(), InteractionHand.OFF_HAND))
            event.getLevel().playSound(null, event.getPos(), ModSounds.SKEWER_DISASSEMBLE.get(),
    ...
    static InteractionResult placePacked(UseOnContext context, List<ItemStack> skewers) {
        ...
        return place(context.getLevel(), context.getClickedPos(), context.getClickedFace(), context.getPlayer(),
                context.getItemInHand(), skewers);
    }

    private static InteractionResult place(Level level, BlockPos clickedPos, Direction face,
                                           net.minecraft.world.entity.player.Player player, ItemStack source,
        ...
        if (!Block.isShapeFullBlock(support.getCollisionShape(level, clickedPos))) return InteractionResult.PASS;
        BlockPos placePos = clickedPos.above();
        if (!level.getBlockState(placePos).canBeReplaced()) return InteractionResult.PASS;
        if (!level.isClientSide) {
            BlockState state = ModBlocks.SKEWER_PLATE.get().defaultBlockState()
                    .setValue(SkewerPlateBlock.FACING, player.getDirection());
            level.setBlock(placePos, state, 3);
            if (level.getBlockEntity(placePos) instanceof SkewerPlateBlockEntity plate) plate.setSkewers(skewers);
