...
public final class ScriptSkewerEatingHandler {
  ...
  public static void onStop(LivingEntityUseItemEvent.Stop event) {
    ...
    result = ForgeEventFactory.onItemUseFinish(event.getEntity(), consumed, 0, result);
    event.getEntity().setItemInHand(event.getEntity().getUsedItemHand(), result);
  ...
  public static void onFinish(LivingEntityUseItemEvent.Finish event) {
    ...
    if (effect != null)
      event.getEntity().addEffect(new MobEffectInstance(effect, data.effectSeconds() * 20));
  }
