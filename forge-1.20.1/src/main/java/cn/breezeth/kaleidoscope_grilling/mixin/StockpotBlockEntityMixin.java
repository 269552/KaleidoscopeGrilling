...
public abstract class StockpotBlockEntityMixin implements SeasonedPotAccess {
    ...
    private void grilling$acceptSeasoning(Level level, LivingEntity user, ItemStack stack, CallbackInfoReturnable<Boolean> cir) {
        ...
        if (user instanceof Player player && !player.getAbilities().instabuild) {
            ...
            if (next >= stack.getMaxDamage()) {
                stack.setCount(0);
                player.setItemInHand(InteractionHand.MAIN_HAND, new ItemStack(ModItems.EMPTY_SEASONING_BOTTLE.get()));
            } else {
    ...
    private void grilling$refreshHotOnTakeout(Level level, LivingEntity user, ItemStack carrier, CallbackInfoReturnable<Boolean> cir) {
