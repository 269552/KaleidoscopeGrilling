...
public final class SkewerColorProvider {
    ...
    public static int color(ItemStack skewer, int tintIndex) {
        ...
        List<ItemStack> ingredients = SkeweringHandler.readIngredientStacks(skewer);
        ...
        PaletteKey key = new PaletteKey(id, ingredient.hasTag() ? ingredient.getTag().hashCode() : 0);
        if (CACHE.size() > 512) CACHE.clear();
        int[] palette = CACHE.computeIfAbsent(key, ignored -> sample(Minecraft.getInstance(), ingredient));
    ...
    private static List<Integer> collect(NativeImage image, int minX, int maxX, int minY, int maxY,
                                         int darkCutoff) {
        ...
        while (clusters.size() > maxColors) {
            ...
            for (int i = 0; i < clusters.size(); i++) {
                for (int j = i + 1; j < clusters.size(); j++) {
                    double distance = labDistanceSquared(clusters.get(i).average(), clusters.get(j).average());
                    if (distance < nearest) {
                        nearest = distance;
                        first = i;
                        second = j;
    ...
    private static int stageColor(ItemStack stack, int rgb, int cell) {
        ...
        if (stage == 0 && ((stack.is(ModItems.SECRET_SKEWER.get()) && SecretSkewerItem.isCooked(stack))
    ...
    private static int luminance(int rgb) {
        return Math.round(((rgb >>> 16) & 0xFF) * 0.2126F
                + ((rgb >>> 8) & 0xFF) * 0.7152F + (rgb & 0xFF) * 0.0722F);
    }
    ...
    private static final class ColorCluster {
        ...
        private ColorCluster(int color) { add(color); }
