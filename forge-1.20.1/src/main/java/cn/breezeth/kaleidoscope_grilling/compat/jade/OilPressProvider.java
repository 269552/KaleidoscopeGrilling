...
enum OilPressProvider implements IBlockComponentProvider {
    ...
    public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
        ...
        tooltip.add(Component.translatable("jade.kaleidoscope_grilling.press." + state,
                state.equals("progress") ? press.progress() : press.cakes(),
                state.equals("progress") ? OilPressBlockEntity.REQUIRED_PROGRESS : OilPressBlockEntity.MAX_CAKES));
        OilPressContainerApi.TransferResult result = OilPressContainerApi.probeNearby(
                accessor.getLevel(), accessor.getPosition(), OilPressBlockEntity.MAX_CAKES);
        BigVatBlockEntity vat = result.containerPos() != null
                && accessor.getLevel().getBlockEntity(result.containerPos()) instanceof BigVatBlockEntity found ? found : null;
        Component container;
        if (result.status() == OilPressContainerApi.TransferStatus.NO_CONTAINER) {
            ...
            container = Component.translatable("jade.kaleidoscope_grilling.press.vat." + key,
                    vat.buckets(), BigVatBlockEntity.CAPACITY_BUCKETS);
        } else {
            String key = result.status() == OilPressContainerApi.TransferStatus.FULL ? "full" : "ready";
    ...
    @Override public ResourceLocation getUid() { return GrillingJadePlugin.PRESS; }
}
