...
enum GrillProvider implements IBlockComponentProvider {
    ...
    if (!skewers.isEmpty()) {
      ...
      tooltip.add(
          Component.translatable(
              accessor.getBlockState().getValue(GrillBlock.LIT)
  ...
  private static String statusKey(BlockAccessor accessor, GrillBlockEntity grill) {
    if (!accessor.getBlockState().getValue(GrillBlock.LIT))
      return "jade.kaleidoscope_grilling.grill.need_heat";
    if (grill.getPhase() == 0) return "jade.kaleidoscope_grilling.grill.need_oil";
    if (grill.getPhase() == 1)
      return grill.getFlipCooldown() > 0
          ? "jade.kaleidoscope_grilling.grill.flipping"
          : "jade.kaleidoscope_grilling.grill.need_flip";
    if (grill.getPhase() == 2)
      return grill.isSeasoned()
  ...
  public ResourceLocation getUid() {
    return GrillingJadePlugin.GRILL;
  }
}
