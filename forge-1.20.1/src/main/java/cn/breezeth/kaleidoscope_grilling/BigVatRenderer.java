...
public final class BigVatRenderer implements BlockEntityRenderer<BigVatBlockEntity> {
    ...
    @Override public void render(BigVatBlockEntity vat, float partialTick, PoseStack pose,
                                 MultiBufferSource buffers, int packedLight, int overlay) {
        FluidStack stack = vat.fluid();
        if (stack.isEmpty()) return;
        int level = Math.min(4, Math.max(1, (vat.amount() * 4 + BigVatBlockEntity.CAPACITY - 1) / BigVatBlockEntity.CAPACITY));
        ...
        var p = pose.last();
        consumer.vertex(p.pose(), 0, 0, SIZE).color(r, g, b, a).uv(u0, v1).overlayCoords(overlay).uv2(light).normal(p.normal(), 0, 1, 0).endVertex();
        consumer.vertex(p.pose(), SIZE, 0, SIZE).color(r, g, b, a).uv(u1, v1).overlayCoords(overlay).uv2(light).normal(p.normal(), 0, 1, 0).endVertex();
        consumer.vertex(p.pose(), SIZE, 0, 0).color(r, g, b, a).uv(u1, v0).overlayCoords(overlay).uv2(light).normal(p.normal(), 0, 1, 0).endVertex();
        consumer.vertex(p.pose(), 0, 0, 0).color(r, g, b, a).uv(u0, v0).overlayCoords(overlay).uv2(light).normal(p.normal(), 0, 1, 0).endVertex();
        pose.popPose();
    }
}
