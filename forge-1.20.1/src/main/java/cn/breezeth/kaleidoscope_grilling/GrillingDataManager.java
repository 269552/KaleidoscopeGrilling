...
public final class GrillingDataManager extends SimpleJsonResourceReloadListener {
    ...
    public record Skewer(List<List<String>> ingredients, String cookedResult, String effect, int effectSeconds) {}

    private static volatile Map<String, Skewer> SKEWERS = Map.of();
    private static volatile Map<String, String> KINDS = Map.of();

    public GrillingDataManager() { super(GSON, "grilling"); }
    public static void register(AddReloadListenerEvent event) { event.addListener(new GrillingDataManager()); }
    public static Skewer skewer(String id) { return SKEWERS.get(id); }
    public static Map<String, Skewer> skewers() { return SKEWERS; }
    ...
    protected void apply(Map<ResourceLocation, JsonElement> files, ResourceManager manager, ProfilerFiller profiler) {
        ...
        files.values().forEach(json -> {
            JsonObject root = json.getAsJsonObject();
            if (root.has("skewers")) for (JsonElement element : root.getAsJsonArray("skewers")) {
                JsonObject object = element.getAsJsonObject();
                List<List<String>> slots = new ArrayList<>();
                for (JsonElement slot : object.getAsJsonArray("ingredients")) {
                    List<String> selectors = new ArrayList<>();
                    for (JsonElement selector : slot.getAsJsonArray()) selectors.add(selector.getAsString());
                    slots.add(List.copyOf(selectors));
                }
                skewers.put(object.get("id").getAsString(), new Skewer(
                        List.copyOf(slots),
            ...
            if (root.has("seasoning_effects")) for (JsonElement element : root.getAsJsonArray("seasoning_effects")) {
                JsonObject object = element.getAsJsonObject();
