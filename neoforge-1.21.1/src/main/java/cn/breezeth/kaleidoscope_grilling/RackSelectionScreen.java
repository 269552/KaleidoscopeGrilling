...
public final class RackSelectionScreen extends AbstractContainerScreen<RackSelectionMenu> {
  ...
  public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
    ...
    double logicalMouseX = logicalMouseX(mouseX, eased),
        logicalMouseY = logicalMouseY(mouseY, eased);
    graphics.pose().pushPose();
    graphics.pose().translate(width / 2F, height / 2F + (1 - eased) * 10, 0);
    float scale = .84F + .16F * eased;
  ...
  public boolean mouseClicked(double mouseX, double mouseY, int button) {
    float p = Mth.clamp(ticksOpen / 9F, 0F, 1F), eased = 1F - (1F - p) * (1F - p) * (1F - p);
    double logicalX = logicalMouseX(mouseX, eased), logicalY = logicalMouseY(mouseY, eased);
