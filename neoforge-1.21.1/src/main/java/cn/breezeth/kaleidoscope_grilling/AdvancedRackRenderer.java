...
public final class AdvancedRackRenderer implements BlockEntityRenderer<AdvancedRackBlockEntity> {
    ...
    @Override
    public void render(AdvancedRackBlockEntity rack, float partialTick, PoseStack pose,
        ...
        for (int slot = 5; slot < 9 && hook < HOOK_X.length; slot++) {
            ...
            context.getItemRenderer().renderStatic(stack, ItemDisplayContext.FIXED, light, overlay,
                    pose, buffers, rack.getLevel(), slot);
            pose.popPose();
            hook++;
        }
        pose.popPose();
