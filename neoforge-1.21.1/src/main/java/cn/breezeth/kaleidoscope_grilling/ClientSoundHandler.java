...
public final class ClientSoundHandler {
    ...
    public static void clientTick(ClientTickEvent.Post event) {
        ...
        boolean shaking = player != null && player.isUsingItem()
    ...
    private static void spawnPremiumOilPotParticles() {
        ...
        if (minecraft.level == null || minecraft.player == null || minecraft.level.getGameTime() % 8L != 0L) return;
        BlockPos center = minecraft.player.blockPosition();
        BlockPos.MutableBlockPos cursor = new BlockPos.MutableBlockPos();
        for (int x = -8; x <= 8; x++) for (int y = -4; y <= 4; y++) for (int z = -8; z <= 8; z++) {
