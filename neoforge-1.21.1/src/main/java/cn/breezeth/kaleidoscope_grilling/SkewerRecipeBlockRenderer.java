...
public final class SkewerRecipeBlockRenderer implements BlockEntityRenderer<SkewerRecipeBlockEntity> {
    ...
    @Override
    public void render(SkewerRecipeBlockEntity recipe, float partialTick, PoseStack pose,
                       MultiBufferSource buffers, int light, int overlay) {
        ResourceLocation id = ResourceLocation.tryParse(recipe.recipeResult());
        if (id == null) return;
        ItemStack result = BuiltInRegistries.ITEM.getOptional(id).map(ItemStack::new).orElse(ItemStack.EMPTY);
