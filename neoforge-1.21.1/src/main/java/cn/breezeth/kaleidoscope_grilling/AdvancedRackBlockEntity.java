...
public final class AdvancedRackBlockEntity extends BaseContainerBlockEntity {
    ...
    public boolean depositMatching(ServerPlayer player) {
        ...
        for (int hotbarSlot = 0; hotbarSlot < 9; hotbarSlot++) {
            ...
            int room = stored.isEmpty() ? carried.getMaxStackSize() : stored.getMaxStackSize() - stored.getCount();
            int move = Math.min(room, carried.getCount());
            if (move <= 0) continue;
            if (stored.isEmpty()) setItem(rackSlot, carried.copyWithCount(move)); else stored.grow(move);
            carried.shrink(move);
    ...
    public boolean swapWithHotbar(ServerPlayer player, int compartment) {
        ...
        if (!held.isEmpty() && previous != null) {
            ...
            if (previousRack != null && (previousRack != this || previous.slot() != compartment)
        ...
        if (!held.isEmpty() && returnRack == null && !canFitInInventory(inventory, hotbarSlot, held)) return false;
    ...
    public static BlockPos selectedBindingPos(ServerPlayer player) {
        ...
        if (!memory.getString("Dim" + hotbarSlot).equals(player.level().dimension().location().toString())) return null;
    ...
    private RackBinding binding(ServerPlayer player, int hotbarSlot) {
        ...
        if (!memory.contains("Pos" + hotbarSlot) || !memory.getString("Dim" + hotbarSlot).equals(dimension)) return null;
        int slot = memory.getInt("Slot" + hotbarSlot);
        return slot >= 0 && slot < COMPARTMENT_COUNT ? new RackBinding(BlockPos.of(memory.getLong("Pos" + hotbarSlot)), slot) : null;
    }
    ...
    private static boolean canFitInInventory(Inventory inventory, int excludedSlot, ItemStack stack) {
        ...
        for (int slot = 0; slot < 36 && remaining > 0; slot++) {
            ...
            else if (ItemStack.isSameItemSameComponents(stored, stack)) remaining -= stored.getMaxStackSize() - stored.getCount();
        }
        return remaining <= 0;
    }
    ...
    private int findMatchingSlotWithRoom(ItemStack stack) {
        for (int i = 0; i < COMPARTMENT_COUNT; i++) {
            ...
            int room = stored.isEmpty() ? stack.getMaxStackSize() : stored.getMaxStackSize() - stored.getCount();
            if (!filter.isEmpty() && canPlaceItem(i, stack) && canShareCategory(filter, stack) && room > 0) return i;
        }
        return -1;
    }
    ...
    private int findReturnSlot(ItemStack stack, int excludedSlot) {
        for (int slot = 0; slot < COMPARTMENT_COUNT; slot++) {
            if (slot != excludedSlot && !filters.get(slot).isEmpty() && canReceiveReturned(slot, stack)) return slot;
        }
        return -1;
    }
    ...
    protected void loadAdditional(CompoundTag tag, HolderLookup.Provider provider) {
        ...
        if (tag.contains("Filters")) ContainerHelper.loadAllItems(tag.getCompound("Filters"), filters, provider);
        for (int i = 0; i < COMPARTMENT_COUNT; i++) {
            if (filters.get(i).isEmpty() && !items.get(i).isEmpty()) filters.set(i, items.get(i).copyWithCount(1));
        }
    }
