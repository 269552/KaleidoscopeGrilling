...
public final class BigVatRenderer implements BlockEntityRenderer<BigVatBlockEntity> {
    ...
    @Override public void render(BigVatBlockEntity vat, float partialTick, PoseStack pose,
        ...
        var p = pose.last();
        consumer.addVertex(p, 0, 0, SIZE).setColor(r, g, b, a).setUv(u0, v1).setOverlay(overlay).setLight(light).setNormal(p, 0, 1, 0);
        consumer.addVertex(p, SIZE, 0, SIZE).setColor(r, g, b, a).setUv(u1, v1).setOverlay(overlay).setLight(light).setNormal(p, 0, 1, 0);
        consumer.addVertex(p, SIZE, 0, 0).setColor(r, g, b, a).setUv(u1, v0).setOverlay(overlay).setLight(light).setNormal(p, 0, 1, 0);
        consumer.addVertex(p, 0, 0, 0).setColor(r, g, b, a).setUv(u0, v0).setOverlay(overlay).setLight(light).setNormal(p, 0, 1, 0);
        pose.popPose();
    }
}
