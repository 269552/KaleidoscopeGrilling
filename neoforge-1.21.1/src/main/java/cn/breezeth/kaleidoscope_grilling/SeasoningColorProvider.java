...
public final class SeasoningColorProvider {
    ...
    public static int color(ItemStack bottle, int tintIndex) {
        ...
        ResourceLocation id = ResourceLocation.parse(ingredients.get(layer));
        int[] colors = CACHE.computeIfAbsent(id, ignored -> sample(new ItemStack(BuiltInRegistries.ITEM.get(id))));
    ...
    private static int[] sample(ItemStack ingredient) {
        try {
            ...
            for (int y = height / 4; y < height * 3 / 4; y++) for (int x = width / 4; x < width * 3 / 4; x++) {
            ...
            int[] result = counts.entrySet().stream().sorted(Map.Entry.<Integer, Integer>comparingByValue().reversed())
                    .mapToInt(Map.Entry::getKey).limit(2).toArray();
            if (result.length == 0) return FALLBACK.clone();
            if (result.length == 1) result = new int[]{result[0], shade(result[0], 0.78F)};
            if (tint >= 0) { result[0] = multiply(result[0], tint); result[1] = multiply(result[1], tint); }
            return result;
        } catch (RuntimeException ignored) { return FALLBACK.clone(); }
    }

    private static int multiply(int a, int b) { return ((a >> 16 & 255) * (b >> 16 & 255) / 255) << 16 | ((a >> 8 & 255) * (b >> 8 & 255) / 255) << 8 | (a & 255) * (b & 255) / 255; }
    private static int shade(int rgb, float f) { return (int)((rgb >> 16 & 255) * f) << 16 | (int)((rgb >> 8 & 255) * f) << 8 | (int)((rgb & 255) * f); }
