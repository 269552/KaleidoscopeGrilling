...
public final class SeasoningData {
    ...
    public static int getVariant(ItemStack stack) { CustomData data=stack.get(DataComponents.CUSTOM_DATA); return data==null?0:net.minecraft.util.Mth.clamp(data.copyTag().getInt(VARIANT_KEY),0,7); }
    public static void setVariant(ItemStack stack,int variant) { CustomData existing=stack.get(DataComponents.CUSTOM_DATA); CompoundTag tag=existing==null?new CompoundTag():existing.copyTag(); tag.putInt(VARIANT_KEY,net.minecraft.util.Mth.clamp(variant,0,7)); stack.set(DataComponents.CUSTOM_DATA,CustomData.of(tag)); }
    public static void setRandomVariant(ItemStack stack) { setVariant(stack,net.minecraft.util.RandomSource.create().nextInt(8)); }
