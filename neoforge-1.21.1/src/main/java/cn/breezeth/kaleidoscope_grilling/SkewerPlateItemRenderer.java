...
public final class SkewerPlateItemRenderer extends BlockEntityWithoutLevelRenderer {
    private static final ResourceLocation PLATE_MODEL = ResourceLocation.fromNamespaceAndPath(
            KaleidoscopeGrilling.MOD_ID, "item/skewer_plate_base");
    private static final float[][] SLOTS = {
            {8F, 3F, 6.25F, 0F}, {12F, 3F, 6.25F, 0F}, {4F, 3F, 6.25F, 0F},
            {6F, 5.5F, 6.25F, -22.5F}, {10F, 5.5F, 6.25F, -22.5F}
    };
    ...
    @Override
    public void renderByItem(ItemStack plate, ItemDisplayContext context, PoseStack pose,
                             MultiBufferSource buffers, int light, int overlay) {
        Minecraft minecraft = Minecraft.getInstance();
        BakedModel plateModel = minecraft.getModelManager().getModel(ModelResourceLocation.standalone(PLATE_MODEL));
        pose.pushPose();
        pose.translate(0.5F, 0.5F, 0.5F);
        minecraft.getItemRenderer().render(plate, ItemDisplayContext.NONE, false, pose, buffers,
                light, overlay, plateModel);
        pose.popPose();
        ...
        for (int i = 0; i < skewers.size() && i < SLOTS.length; i++) {
            ...
            minecraft.getItemRenderer().renderStatic(skewer, ItemDisplayContext.FIXED,
                    light, overlay, pose, buffers, minecraft.level, i);
            pose.popPose();
        }
    }
