...
public final class GrillRenderer implements BlockEntityRenderer<GrillBlockEntity> {
    ...
    @Override public void render(GrillBlockEntity grill,float partial,PoseStack pose,MultiBufferSource buffers,int light,int overlay){
        GrillLoopSound.ensure(grill);
        int facingRotation=switch(grill.getBlockState().getValue(GrillBlock.FACING)){case EAST->90;case SOUTH->180;case WEST->270;default->0;};
        pose.pushPose();pose.translate(0.5,0,0.5);pose.mulPose(Axis.YP.rotationDegrees(-facingRotation));pose.translate(-0.5,0,-0.5);
        GrillBlockEntity.FlipAnimationData animation=grill.flipAnimationData;
        int flips=grill.getFlips();
        long now=System.currentTimeMillis();
        if(animation.observedFlips<0){animation.observedFlips=flips;}
        else if(animation.observedFlips!=flips){
            animation.observedFlips=flips;animation.timestamp=now;
            RandomSource random=RandomSource.create(grill.getBlockPos().asLong()^((long)flips*341873128712L));
            for(int i=0;i<animation.heights.length;i++)animation.heights[i]=0.28F+random.nextFloat()*0.10F;
        }
        float progress=animation.timestamp<0?1F:Mth.clamp((now-animation.timestamp)/700F,0F,1F);
        ...
        for(int i=0;i<grill.getContainerSize();i++){
            ...
            if(stack.is(ModItems.SECRET_SKEWER.get())||SkewerRecipes.isRawSkewer(stack)||SkewerRecipes.isCookedSkewer(stack)){
                ...
                Minecraft.getInstance().getItemRenderer().renderStatic(stack,ItemDisplayContext.NONE,light,overlay,pose,buffers,grill.getLevel(),i);
            }else{
                pose.translate((3F+i*5F)/16F,0.30F+lift,0.50F);
                pose.mulPose(Axis.XP.rotationDegrees(90));
                pose.mulPose(Axis.YP.rotationDegrees(rotation));
                Minecraft.getInstance().getItemRenderer().renderStatic(stack,ItemDisplayContext.FIXED,light,overlay,pose,buffers,grill.getLevel(),i);
            }
            pose.popPose();
        }
        pose.popPose();
