...
public final class InvincibleHandler {
  ...
  public static void onDamage(LivingIncomingDamageEvent event) {
    ...
    if (!entity.hasEffect(ModEffects.INVINCIBLE) || event.getSource().is(DamageTypes.GENERIC_KILL)) return;
  ...
  public static void onPlayerTick(PlayerTickEvent.Post event) {
    ...
    if (player.tickCount % 10 != 0 || !player.hasEffect(ModEffects.INVINCIBLE) || !(player.level() instanceof ServerLevel level)) return;
