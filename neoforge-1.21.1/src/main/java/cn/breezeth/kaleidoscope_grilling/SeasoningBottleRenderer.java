...
public final class SeasoningBottleRenderer implements BlockEntityRenderer<SeasoningBottleBlockEntity> {
    private static final float[][][] OFFSETS = {
            {{0, 0}}, {{4, 3}, {-3, -1}}, {{4, 4}, {-3, 3}, {3, -3.75F}},
            {{4, 5}, {-3, 4}, {3.5F, -3.25F}, {-4.25F, -3.25F}}};
    public SeasoningBottleRenderer(BlockEntityRendererProvider.Context context) {}
    @Override public void render(SeasoningBottleBlockEntity bottle, float partialTick, PoseStack pose,
        ...
        for (int i = 0; i < stacks.size(); i++) {
            ...
            Minecraft.getInstance().getItemRenderer().renderStatic(stacks.get(i), ItemDisplayContext.NONE, light, overlay,
                    pose, buffers, bottle.getLevel(), i);
            pose.popPose();
        }
        pose.popPose();
    }
