...
public final class GrillCommand {
            ...
            Commands.literal("kg")
                .then(
                    Commands.literal("grill")
                        .then(Commands.literal("unlock").executes(context -> unlock(context.getSource().getPlayerOrException())))
                        .then(
  ...
  private static int unlock(ServerPlayer player) {
    ...
    if (!(hit instanceof BlockHitResult blockHit)
        || !(player.level().getBlockEntity(blockHit.getBlockPos()) instanceof GrillBlockEntity)) {
      player.displayClientMessage(
          Component.translatable("command.kaleidoscope_grilling.grill.not_found"), true);
      return 0;
    }
    boolean unlocked =
        GrillAutomationApi.forceUnlock(player.level(), blockHit.getBlockPos());
    player.displayClientMessage(
        Component.translatable(
            unlocked
